...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  Future<void> _getDailyExpenses(String selectedDate) async {
    final response = await http.get(
        Uri.parse(
            'http://192.168.1.122:8080/api/getDailyExpenses?date=$selectedDate'),
  ...
  Future<void> _getWeeklyPurchaseInvoices() async {
    final response = await http.get(
      Uri.parse(
          'http://192.168.1.122:8080/api/getWeeklyPurchaseInvoices?startDate=${dateController.text}&endDate=${dateController2.text}'),
      headers: <String, String>{
        'Authorization': 'Bearer ${await getTokenFromLocalStorage()}'
      },
    );
